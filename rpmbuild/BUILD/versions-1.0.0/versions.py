...
def get_file_tracking_head(abs_filepath):
    ...
    if len(db_search) == 0:
        print(
            f"No versions of {os.path.basename(abs_filepath)} are currently being tracked by versions."
        )
        return None
    return db_search
...
if __name__ == "__main__":
    ...
    if rel_filepath is None:
        print(
            'No file specified. Use the --file flag to input a file. Run "versions --help" for more info.'
        )
        exit()
    ...
    if cmd == "rs" or cmd == "restore":
        ...
        if hexsha is None:
            print(
                f"Tag value is required for versions to restore your file.\n"
